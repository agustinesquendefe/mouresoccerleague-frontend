...
import { getRefereeAssignedMatches } from '@/services/refereePortal/getRefereeAssignedMatches';
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Unable to load referee matches.' },
      { status: 500 }
    );
  }
}
