...
begin
  ...
  select exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'profiles' and column_name = 'portal_role'
  ...
  portal_role := new.raw_user_meta_data ->> 'portal_role';
  ...
  if has_portal_role then
    insert_columns := insert_columns || ', portal_role';
  ...
  using new.id, new.email, nullif(display_name, ''), 'viewer', portal_role;
